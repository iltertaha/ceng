...
# 2236891
#
# I've added IADDQ into the valid instruction.After that,I've added
# into the need_regids because one operand is register.In addition 
# since another operator is immediate, I added IIADDQ into need_valC.
# Besides,since the second operand is register, decoded register is
# used for rB in srcB.The same register is also used as destination of
# ALU operation.So,I've set d_dstE is D_rB for IIADDQ op.
# I passed immediate value from execute stage to input A of the alu.
# Another input of ALU should be valB to do an addition with immediate
# and register.
# After this operation, sum is stored in the same register.
#
# Moreover, IADDQ should set condition codes since it is arithmetic/math
# operation.So I modified set_cc and added IIADDQ.
